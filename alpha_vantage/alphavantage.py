...
class AlphaVantage(object):
    ...
    def __init__(self, key=None, retries=5, output_format='json',
        ...
        if not key or not isinstance(key, str):
            raise ValueError('The AlphaVantage API key must be provided either through the key parameter or '
                             'through the environment variable ALPHAVANTAGE_API_KEY.')
        self.supported_formats = ["csv", "pandas", "json"]
        self.key = key
        ...
        # Not all the calls accept a data type appended at the end
        # This variable will be overriden by those functions not needing it.
    ...
    def _call_api_on_func(cls, func):
        ...
        def _call_wrapper(self, *args, **kwargs):
            ...
            # The original function called must return the function name defined in the AlphaVantage API and the data
            # key for it and for its meta data.
            function_name, data_key, meta_data_key = func(self, *args, **kwargs)
            url = f'{AlphaVantage._ALPHA_VANTAGE_API_URL}function={function_name}'
            for idx, arg_name in enumerate(argspec.args[1:]):
                ...
                if 'matype' in arg_name and arg_value:
                    # If the argument name has matype, we gotta map the string or the integer
                    arg_value = self.map_to_matype(arg_value)
                if arg_value:
                    # Discard argument in the url formation if it was set to None
                    ...
                    url = f'{url}&{arg_name}={arg_value}'
            # Allow the output format to be JSON / csv (supported by AlphaVantage API). Pandas is simply converted JSON.
            if 'json' in self.output_format.lower() or 'csv' in self.output_format.lower():
                ...
                raise ValueError(f'Output format: {self.output_format.lower()} not recognized,'
                                 f'only {",".join(self.supported_formats)} are supported')
            if self._append_type:
                url = f'{url}&apikey={self.key}&datatype={oformat}'
            else:
            ...
            if 'json' in self.output_format.lower() or 'pandas' in self.output_format.lower():
                ...
                if output_format == 'json':
                    ...
                    if isinstance(data, list):
                        # If the call returns a list, then we will append them in the resulting data frame.
                        # If in the future AlphaVantage decides to do more with ret arrays, this might become buggy.
                        data_array = []
                        for val in data:
    ...
    @staticmethod
    def map_to_matype(matype):
        """ Convert to the alpha vantage math type integer.
        It returns an integer correspondant to the type of math to apply to a function.
        It raises ValueError if an integer greater than the supported math types is given.
    ...
    def _handle_api_call(self, url):
        """ Handle the return call from the API and return a data and meta_data object.
